...
export async function GET(request: NextRequest) {
  ...
  try {
    const runs = await recentWorkflowRuns(MARKET_WORKFLOW, token);
    ...
    await dispatchWorkflow(MARKET_WORKFLOW, token, { trigger_source: "vercel-cron" });
    return NextResponse.json(
    ...
    return NextResponse.json(
      { accepted: false, error: "Unable to dispatch market workflow" },
      { status: 502 },
    );
  }
}
