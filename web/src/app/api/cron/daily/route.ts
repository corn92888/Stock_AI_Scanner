...
export async function GET(request: NextRequest) {
  ...
  try {
    const runs = await recentWorkflowRuns(DAILY_WORKFLOW, token);
    ...
    await dispatchWorkflow(DAILY_WORKFLOW, token, { trigger_source: "vercel-cron" });
    return NextResponse.json(
    ...
    return NextResponse.json(
      { accepted: false, error: "Unable to dispatch daily workflow" },
      { status: 502 },
    );
  }
}
