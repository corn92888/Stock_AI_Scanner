...
export async function GET(request: NextRequest) {
  ...
  try {
    const runs = await recentWorkflowRuns(INSTITUTIONAL_WORKFLOW, token);
    ...
    await dispatchWorkflow(INSTITUTIONAL_WORKFLOW, token, {
    ...
    return NextResponse.json(
      { accepted: false, error: "Unable to dispatch institutional workflow" },
      { status: 502 },
    );
  }
}
